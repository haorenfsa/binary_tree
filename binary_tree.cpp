...
void btree::insert(INDEX index)
{
    // this function must insert the value in the tree
	...
	if( NULL != rootNode )
	{
		if( search(index) )
			return;
		// if d is not in the tree already, create a new node with data equal index
		else
		{
		
			    // if the tree is not empty, look for the right place where to insert index
				...
				//pre-order search for first NULL
				// TODO:
		}
	}
	else	// if the tree is empty, set the root to the new node
	{
		rootNode = new node;
...
bool btree::search(INDEX index)
{
    // This function must call the private function
    // search_element(node*,int) passing the root and
    // index as parameters. The function
    // must use recursion.
	return search_element(root, index);
}
...
// The function must use recursion.
bool btree::search_element(node* top, INDEX index)
{
	//use preorder
	if( NULL != top )//only top need NULL check
	{
		if(top->index != index)//root
		{
			if( !search_element(top->left, index) )	//left
				return search_element(top->right, index);	//right
			else
				return true;
		}
		else//found in root
